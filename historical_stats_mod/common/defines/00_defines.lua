...
NDiplomacy = {
	...
	VOLUNTEERS_TRANSFER_SPEED = 45,					-- days to transfer a unit to another nation
	VOLUNTEERS_DIVISIONS_REQUIRED = 50,				-- This many divisons are required for the country to be able to send volunteers.
	...
	TENSION_VOLUNTEER_FORCE_DIVISION = 0.15,			-- Amount of tension generated for each sent division
	...
	VICTORY_POINT_WORTH_FACTOR = 65,				-- multiplier when calcualting proince worth (surrender)
...
NProduction = {
	...
	BASE_FACTORY_EFFICIENCY_GAIN = 0.90,		-- Base efficiency gain for factories per unit produced in %. 1.00
	BASE_FACTORY_EFFICIENCY_BALANCE_FACTOR = 0.1, 			-- Factory efficiency balancing factor
	BASE_FACTORY_EFFICIENCY_VARIANT_CHANGE_FACTOR = 92,		-- Base factor for changing production variants in %. 90
	BASE_FACTORY_EFFICIENCY_PARENT_CHANGE_FACTOR = 86,		-- Base factor for changing production parent<->children in %. 72
	BASE_FACTORY_EFFICIENCY_FAMILY_CHANGE_FACTOR = 55,		-- Base factor for changing production with same family in %. 55
	BASE_FACTORY_EFFICIENCY_ARCHETYPE_CHANGE_FACTOR = 35, 	-- Base factor for changing production with same archetype in %. 35
...
NBuildings = {
	...
	MAX_SHARED_SLOTS = 32,				-- Max slots shared by factories
...
NMilitary = {
	...
	LAND_COMBAT_ORG_ARMOR_DEFLECTION_FACTOR = 0.55, -- damage reduction if armor outclassing enemy
	LAND_COMBAT_COLLATERAL_FACTOR = 0.007,		   -- Factor to scale collateral damage to infra and forts with.
	LAND_COMBAT_FORT_DAMAGE_CHANCE = 0,		-- chance to get a hit to damage on forts. (out of 100)
	ATTRITION_DAMAGE_ORG = 0.12,					   -- damage from attrition to Organisation
	ATTRITION_EQUIPMENT_LOSS_CHANCE = 0.15,		   -- Chance for loosing equipment when suffer attrition. Scaled up the stronger attrition is. Then scaled down by equipment reliability.
	...
	ATTRITION_WHILE_MOVING_FACTOR = 0.1,
	...
	RIVER_CROSSING_PENALTY_LARGE = -0.7,           -- large river crossing
	RIVER_CROSSING_SPEED_PENALTY = -0.35,          -- small river crossing
	RIVER_CROSSING_SPEED_PENALTY_LARGE = -0.7,     -- large river crossing
	...
	ENEMY_AIR_SUPERIORITY_DEFENSE_STEEPNESS = 70, -- how quickly defense approaches the max impact diminishing returns curve
	ENEMY_AIR_SUPERIORITY_SPEED_IMPACT = -0.35,     -- effect on speed due to enemy air superiority
	...
	UNIT_EXPERIENCE_PER_COMBAT_HOUR = 0.0005,		-- was 0.0001, less rewarding than fight 18 hours ***
	...
	UNIT_EXP_LEVELS = { 0.1, 0.3, 0.55, 0.85 },		-- Experience needed to progress to the next level Changed vets to 0.55 instead 0.7 ***
	...
	EXPERIENCE_LOSS_FACTOR = 0.7,                 -- percentage of experienced solders who die when manpower is removed	Was 1.0 *** 
	...
	ARMOR_VS_AVERAGE = 0.44,			                -- how to weight in highest armor & pen vs the division average ***
	PEN_VS_AVERAGE = 0.36,								 -- how to weight in highest armor & pen vs the division average ***
...
NAir = {
	...
	COMBAT_MAX_WINGS_AT_ONCE = 8, 						-- Max amount of air wings in one combat simulation. The higher value, the quicker countries may loose their wings. It's a gameplay balance value.
	COMBAT_MAX_WINGS_AT_GROUND_ATTACK = 55,	        	-- we can really pounce a land strike and escalate
	COMBAT_MAX_WINGS_AT_ONCE_PORT_STRIKE = 400,         -- DNM - original 1000 -- we can really pounce a naval strike and escalate
...
NNavy = {
	...
	DETECTION_SUBS_CHANCE_BALANCE = 12.0,						-- Same as above, but for sub detection. It should be very hard to detect just the subs.
	...
	REPAIR_AND_RETURN_PRIO_LOW_COMBAT = 0.3,						-- DNM, original 0.1, BICE had 0.25 -- % of total Strength. When below, navy will go to home base to repair (in combat).
	...
	CONVOY_EFFICIENCY_LOSS_MODIFIER = 0.8,							-- How much efficiency drops when losing convoys. If modifier is 0.5, then losing 100% of convoys in short period, the efficiency will drop by 50%.
	...
	SUPPLY_NEED_FACTOR = 7,										    -- multiplies supply usage
...
NAI = {
	...
	DIPLOMACY_SEND_EXPEDITIONARY_INDUSTRY_FACTOR = 1.0, -- Weight when counting industry (will be added to military strength)
	DIPLOMACY_SEND_EXPEDITIONARY_BASE = 1,		-- Multiplied with relative strength factor to get result (if other is twice as "strong" the result with 25 would be 50)
	DIPLOMACY_ACCEPT_VOLUNTEERS_BASE = 45,		-- Base value of volunteer acceptance (help is welcome)
	...
	RESEARCH_AHEAD_OF_TIME_FACTOR = 3.0, 		-- To which extent AI should care about ahead of time penalties to research
	...
	AIR_CAS_SCORE_FACTOR = 35.0,				-- Factor applied to (friendly units) x (enemy units)) to get CAS score for region
	...
	AIR_MIN_REASSIGN_SCORE = 85,				-- Minimum total score for region to be considered for reassigning air missions
	...
	LOCATION_BALANCE_TO_ADVANCE = 0.4,			-- Limit on location strength balance between country and enemy for unit to dare to move forward.
	...
	NEIGHBOUR_WAR_THREAT_FACTOR = 1.20, 		-- How much increase in threat does AI feel against neighbours who are at war
	...
	UNIT_ASSIGNMENT_TERRAIN_IMPORTANCE = 2.0,	-- Terrain score for units are multiplied by this when the AI is deciding which front they should be assigned to
	...
	START_TRAINING_EQUIPMENT_LEVEL = 0.98,               -- ai will not start to train if equipment drops below this level
	...
	ATTACK_HEAVILY_DEFENDED_LIMIT = 0.00,				-- AI will not launch attacks against heavily defended fronts unless they consider to have this level of advantage (1.0 = 100%)
	HOUR_BAD_COMBAT_REEVALUATE = 45,                   -- if we are in combat for this amount and it goes shitty then try skipping it 
	...
	DESIRED_UNITS_FACTOR_INVASION_ORDER = 1.1,			-- Factor for desired number of units to assign to naval invasion orders
	...
	STR_BOMB_MILITARY_FACTORY_IMPORTANCE = 90,			-- Strategic importance of enemy military factories
	...
	STR_BOMB_MIN_ENEMY_FIGHTERS_IN_AREA = 450,		-- If amount of enemy fighters is higher than this mission won't perform
	STR_BOMB_FIGHTERS_PER_PLANE = 1.1,					-- Amount of air superiority planes requested per enemy plane
	STR_BOMB_PLANES_PER_CIV_FACTORY = 25,				-- Amount of planes requested per enemy civ factory
	STR_BOMB_PLANES_PER_MIL_FACTORY = 35,			-- Amount of planes requested per enemy military factory
	STR_BOMB_PLANES_PER_NAV_FACTORY = 18,				-- Amount of planes requested per enemy naval factory
