...
NProduction = {
	...
	BASE_FACTORY_SPEED_MIL = 4.3, 				-- Base factory speed multiplier (how much hoi3 style IC each factory gives).
...
NMilitary = {
	...
	UNIT_EXPERIENCE_PER_COMBAT_HOUR = 0.0002,		-- was 0.0001, 3x need test ***
	...
	PLAN_CONSIDERED_GOOD = 0.8,					-- Plan evaluations above this value are considered more or less safe
...
NAir = {
	...
	NAVAL_STRIKE_DAMAGE_TO_STR = 2.6,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Strength reduction.
	NAVAL_STRIKE_DAMAGE_TO_ORG = 2.7,					-- Balancing value to convert damage ( naval_strike_attack * hits ) to Organisation reduction.
...
NNavy = {
	...
	COMBAT_MIN_DURATION = 7,										-- Min combat duration before we can retreat. It's a balancing variable so it's not possible to always run with our weak ships agains big flotillas.
	...
	SUBMARINE_REVEAL_BASE_CHANCE = 0.04,							-- Base chance for submarine detection. It's modified by the difference of a spootter's submarines detection vs submarine visibility. Use this variable for game balancing.
	...
	ENEMY_AIR_SUPERIORITY_IMPACT = -0.70,           					-- effect on ship efficiency due to enemy air superiorty
...
NAI = {
	...
	DIPLOMACY_ACCEPT_VOLUNTEERS_BASE = 40,		-- Base value of volunteer acceptance (help is welcome)
	...
	SEND_VOLUNTEER_EVAL_MAJOER_POWER = 1.3, 	-- How willing major powers are to send volunteers.
	...
	RELATIVE_STRENGTH_TO_INVADE_DEFENSIVE = 1.4,-- Compares the estimated strength of the country/faction compared to it's enemies to see if it should invade or stay at home to defend, but while being a defensive country.
	...
	UPGRADE_DIVISION_RELUCTANCE = 18,					-- How often to consider upgrading to new templates for units in the field
	...
	ATTACK_HEAVILY_DEFENDED_LIMIT = 0.95,				-- AI will not launch attacks against heavily defended fronts unless they consider to have this level of advantage (1.0 = 100%)
	HOUR_BAD_COMBAT_REEVALUATE = 18,                   -- if we are in combat for this amount and it goes shitty then try skipping it 
